...
function musicRuntime(busDuration, songDurations) {
    ...
    for (let i = 0; i < songDurations.length; i++) {
        for (let j = i + 1; j < songDurations.length; j++) {
            let length = songDurations[i] + songDurations[j];
            if (length === busDuration - 30) {
                if (Math.max(songDurations[i], songDurations[j] > longestSong));
                longestSong = Math.max(songDurations[i], songDurations[j]);
                songPair = [i, j];
            }
        }
    }
